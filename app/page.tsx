...
const fields: Record<ModuleKey, Field[]> = {
  ...
  onpage: [{ key: "url", label: "URL", required: true }, ...["https", "canonical", "indexable", "title", "meta", "h1", "headings", "intent", "internal", "external", "alt", "schema", "mobile"].map(key => ({ key, label: key.toUpperCase(), type: "checkbox" as const })), { key: "missing", label: "Việc còn thiếu", type: "textarea" }],
...
const moduleMeta: Record<ModuleKey, { title: string; eyebrow: string; description: string; columns: string[]; prefix: string }> = {
  ...
  content: { title: "Kế hoạch nội dung", eyebrow: "CONTENT OPERATIONS", description: "Quản lý keyword, topic cluster, tiến độ và điểm cơ hội.", columns: ["topic", "keyword", "intent", "cluster", "publishDate", "score", "status"], prefix: "CONTENT" },
  ...
  onpage: { title: "On-page Checklist", eyebrow: "URL QUALITY CONTROL", description: "Chấm điểm từng URL và phát hiện hạng mục còn thiếu.", columns: ["url", "title", "meta", "h1", "internal", "alt", "schema", "score"], prefix: "ONPAGE" },
  audits: { title: "Technical SEO Audit", eyebrow: "TECHNICAL SEO", description: "Theo dõi lỗi, mức ảnh hưởng và tiến độ khắc phục.", columns: ["url", "category", "issue", "severity", "impact", "due", "status"], prefix: "AUDIT" },
  indexing: { title: "Index Tracking", eyebrow: "INDEXATION", description: "Quản lý submit, trạng thái index và lịch kiểm tra 3/7/14 ngày.", columns: ["url", "type", "submitted", "status", "reason", "next", "priority"], prefix: "INDEX" },
  backlinks: { title: "Backlinks", eyebrow: "OFF-PAGE SEO", description: "Quản lý nguồn link, anchor, chất lượng và trạng thái.", columns: ["domain", "targetUrl", "anchor", "linkType", "authority", "score", "status"], prefix: "BL" },
  entities: { title: "Entity SEO", eyebrow: "BRAND ENTITY", description: "Theo dõi hồ sơ thương hiệu, NAP, xác minh và index.", columns: ["name", "type", "platform", "nap", "verified", "indexed", "updated"], prefix: "ENTITY" },
  seeding: { title: "Seeding", eyebrow: "DISTRIBUTION", description: "Theo dõi bài seeding, tài khoản, link đích và tình trạng tồn tại.", columns: ["platform", "postUrl", "targetUrl", "posted", "status", "removed", "checked"], prefix: "SEED" },
  rankings: { title: "Keyword Rankings", eyebrow: "SEARCH PERFORMANCE", description: "Quản lý vị trí, clicks, impressions và CTR.", columns: ["keyword", "page", "position", "previous", "clicks", "impressions", "ctr", "date"], prefix: "KW" },
  worklogs: { title: "Nhật ký làm việc", eyebrow: "DAILY EXECUTION", description: "Ghi thời gian, kết quả, khó khăn và bước tiếp theo.", columns: ["date", "taskId", "group", "description", "hours", "completion", "result"], prefix: "LOG" },
...
function ModuleView({ module, rows, setRows, onChange, siteUrl, personnel }: { module: ModuleKey; rows: Row[]; setRows: (rows: Row[]) => void; onChange: (action: string, entity: string, detail?: string) => void; siteUrl: string; personnel: Row[] }) {
  ...
  return <>
    ...
    {open && <EditorModal title={`${editing ? "Sửa" : "Thêm"} ${meta.title}`} module={module} row={editing || { id: uid(meta.prefix) }} personnel={personnel} onSave={save} onClose={() => { setOpen(false); setEditing(null); }} />}
  </>;
}
...
function EditorModal({ title, module, row, personnel, onSave, onClose }: { title: string; module: ModuleKey; row: Row; personnel: Row[]; onSave: (row: Row) => void; onClose: () => void }) {
  const [draft, setDraft] = useState<Row>({ ...row });
  const submit = (event: React.FormEvent) => { event.preventDefault(); onSave(draft); };
  return <div className="modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><form className="modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">DATA EDITOR</p><h2>{title}</h2></div><button type="button" onClick={onClose}>×</button></div><div className="form-grid">{fields[module].map(field => <label key={field.key} className={field.type === "textarea" ? "wide" : ""}>{field.type === "checkbox" ? <span className="checkbox-label"><input type="checkbox" checked={Boolean(draft[field.key])} onChange={event => setDraft({ ...draft, [field.key]: event.target.checked })} /> {field.label}</span> : <><span>{field.label}{field.required && " *"}</span>{field.key === "owner" ? <select value={String(draft[field.key] ?? "")} onChange={event => setDraft({ ...draft, [field.key]: event.target.value })}><option value="">Chưa gán</option>{personnel.filter(person => String(person.status || "Active") !== "Inactive").map(person => <option key={person.id} value={String(person.name)}>{String(person.name)}{person.role ? ` — ${person.role}` : ""}</option>)}</select> : field.type === "select" ? <select required={field.required} value={String(draft[field.key] ?? field.options?.[0] ?? "")} onChange={event => setDraft({ ...draft, [field.key]: event.target.value })}>{field.options?.map(option => <option key={option}>{option}</option>)}</select> : field.type === "textarea" ? <textarea required={field.required} value={String(draft[field.key] ?? "")} onChange={event => setDraft({ ...draft, [field.key]: event.target.value })} /> : <input required={field.required} type={field.type || "text"} step={field.type === "number" ? "any" : undefined} value={String(draft[field.key] ?? "")} onChange={event => setDraft({ ...draft, [field.key]: field.type === "number" ? Number(event.target.value) : event.target.value })} />}</>}</label>)}</div><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Hủy</button><button className="primary" type="submit">Lưu dữ liệu</button></div></form></div>;
}
