...
function Badge({ value }: { value: string }) {
  ...
  const tone = ["critical", "high", "error", "lost", "removed", "risk", "cancelled", "not indexed", "crawled - not indexed", "discovered - not indexed", "inactive"].some(item => lower === item)
    ? "bad"
    : ["done", "đã xong", "published", "indexed", "live", "verified", "complete", "active"].includes(lower)
      ? "good"
      : ["in progress", "đang làm", "updating", "contacted"].includes(lower)
        ? "progress"
        : ["review", "waiting", "scheduled", "pending check"].includes(lower)
          ? "review"
          : ["medium", "open", "accepted"].includes(lower)
            ? "warn"
            : "neutral";
